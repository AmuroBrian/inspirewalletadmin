...
export default function TransactionList() {
  ...
  const saveNewTransaction = async () => {
    ...
    try {
      const transactionRef = collection(db, `users/${selectedUser}/${modalType}`);
...
>
  ...
  />
      ...
      <p className="mt-2 text-gray-700 font-semibold text-lg ml-56">Loading...</p>
    </div>
  );
}
  ...
  <div className="fixed inset-0 flex items-center justify-center bg-black/30 backdrop-blur-lg">
    <div className="bg-white p-4 rounded-lg w-[70%] shadow-lg border border-gray-300 relative ml-60">
      ...
      {/* Transactions Table */}
      <div className={`overflow-x-auto ${transactions.length > 3 ? "max-h-60 overflow-y-auto" : ""}`}>
